<?php
/*
 * Copyright (c) 2017-2018 THL A29 Limited, a Tencent company. All Rights Reserved.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *    http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
namespace TencentCloud\Cfw\V20190904\Models;
use TencentCloud\Common\AbstractModel;

/**
 * DescribeEnterpriseSecurityGroupRule返回参数结构体
 *
 * @method string getPageNo() 获取分页查询时，显示的当前页的页码。
 * @method void setPageNo(string $PageNo) 设置分页查询时，显示的当前页的页码。
 * @method string getPageSize() 获取分页查询时，显示的每页数据的最大条数。
 * @method void setPageSize(string $PageSize) 设置分页查询时，显示的每页数据的最大条数。
 * @method array getRules() 获取访问控制策略列表
 * @method void setRules(array $Rules) 设置访问控制策略列表
 * @method string getTotalCount() 获取访问控制策略的总数量。
 * @method void setTotalCount(string $TotalCount) 设置访问控制策略的总数量。
 * @method string getRequestId() 获取唯一请求 ID，每次请求都会返回。定位问题时需要提供该次请求的 RequestId。
 * @method void setRequestId(string $RequestId) 设置唯一请求 ID，每次请求都会返回。定位问题时需要提供该次请求的 RequestId。
 */
class DescribeEnterpriseSecurityGroupRuleResponse extends AbstractModel
{
    /**
     * @var string 分页查询时，显示的当前页的页码。
     */
    public $PageNo;

    /**
     * @var string 分页查询时，显示的每页数据的最大条数。
     */
    public $PageSize;

    /**
     * @var array 访问控制策略列表
     */
    public $Rules;

    /**
     * @var string 访问控制策略的总数量。
     */
    public $TotalCount;

    /**
     * @var string 唯一请求 ID，每次请求都会返回。定位问题时需要提供该次请求的 RequestId。
     */
    public $RequestId;

    /**
     * @param string $PageNo 分页查询时，显示的当前页的页码。
     * @param string $PageSize 分页查询时，显示的每页数据的最大条数。
     * @param array $Rules 访问控制策略列表
     * @param string $TotalCount 访问控制策略的总数量。
     * @param string $RequestId 唯一请求 ID，每次请求都会返回。定位问题时需要提供该次请求的 RequestId。
     */
    function __construct()
    {

    }

    /**
     * For internal only. DO NOT USE IT.
     */
    public function deserialize($param)
    {
        if ($param === null) {
            return;
        }
        if (array_key_exists("PageNo",$param) and $param["PageNo"] !== null) {
            $this->PageNo = $param["PageNo"];
        }

        if (array_key_exists("PageSize",$param) and $param["PageSize"] !== null) {
            $this->PageSize = $param["PageSize"];
        }

        if (array_key_exists("Rules",$param) and $param["Rules"] !== null) {
            $this->Rules = [];
            foreach ($param["Rules"] as $key => $value){
                $obj = new SecurityGroupRule();
                $obj->deserialize($value);
                array_push($this->Rules, $obj);
            }
        }

        if (array_key_exists("TotalCount",$param) and $param["TotalCount"] !== null) {
            $this->TotalCount = $param["TotalCount"];
        }

        if (array_key_exists("RequestId",$param) and $param["RequestId"] !== null) {
            $this->RequestId = $param["RequestId"];
        }
    }
}
