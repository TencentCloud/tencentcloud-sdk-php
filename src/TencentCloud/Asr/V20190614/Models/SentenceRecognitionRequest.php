 ...
 * @method string getReplaceTextId() 获取替换词id。用于调用对应的替换词表。
 * @method void setReplaceTextId(string $ReplaceTextId) 设置替换词id。用于调用对应的替换词表。
 */
...
• 热词权重设置为100时，当前热词开启热词增强同音替换功能（仅支持8k_zh,16k_zh），举例：热词配置“蜜制|100”时，与“蜜制”同拼音（mizhi）的“秘制”的识别结果会被强制替换成“蜜制”。因此建议客户根据自己的实际情况开启该功能。建议仅将重要且必须生效的热词设置到100，设置过多权重为100的热词将影响整体字准率。
     ...
     * @param string $ReplaceTextId 替换词id。用于调用对应的替换词表。
     */
