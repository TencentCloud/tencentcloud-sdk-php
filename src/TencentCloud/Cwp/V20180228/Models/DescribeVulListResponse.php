...
注意：此字段可能返回 null，表示取不到有效值。
    ...
    public function deserialize($param)
    {
        ...
        if (array_key_exists("VulInfoList",$param) and $param["VulInfoList"] !== null) {
            $this->VulInfoList = [];
            foreach ($param["VulInfoList"] as $key => $value){
                $obj = new VulInfoList();
                $obj->deserialize($value);
                array_push($this->VulInfoList, $obj);
            }
        }
        ...
        if (array_key_exists("FollowVulCount",$param) and $param["FollowVulCount"] !== null) {
            $this->FollowVulCount = $param["FollowVulCount"];
        }
