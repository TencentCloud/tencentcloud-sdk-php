 ...
 *
 * @method array getMCPs() 获取<p>MCP 列表</p>
注意：此字段可能返回 null，表示取不到有效值。
 * @method void setMCPs(array $MCPs) 设置<p>MCP 列表</p>
注意：此字段可能返回 null，表示取不到有效值。
...
注意：此字段可能返回 null，表示取不到有效值。
    ...
    public function deserialize($param)
    {
        ...
        if (array_key_exists("MCPs",$param) and $param["MCPs"] !== null) {
            $this->MCPs = [];
            foreach ($param["MCPs"] as $key => $value){
                $obj = new MCPInfo();
                $obj->deserialize($value);
                array_push($this->MCPs, $obj);
            }
        }
