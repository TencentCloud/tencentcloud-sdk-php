 ...
 * @method string getDocumentRecognizeResultUrl() 获取文档解析结果的临时下载地址。文件类型为zip压缩包，下载链接有效期30分钟。压缩包内包含*.md、*.json以及images文件夹。

 * @method void setDocumentRecognizeResultUrl(string $DocumentRecognizeResultUrl) 设置文档解析结果的临时下载地址。文件类型为zip压缩包，下载链接有效期30分钟。压缩包内包含*.md、*.json以及images文件夹。

 * @method array getFailedPages() 获取文档解析失败的页码。
 * @method void setFailedPages(array $FailedPages) 设置文档解析失败的页码。
 * @method integer getFailPageNum() 获取文档解析失败页数
 * @method void setFailPageNum(integer $FailPageNum) 设置文档解析失败页数
 * @method integer getSuccessPageNum() 获取文档解析成功页数
 * @method void setSuccessPageNum(integer $SuccessPageNum) 设置文档解析成功页数
...
class ReconstructDocumentSSEResponse extends AbstractModel
{
    ...
    /**
     * @var string 文档解析结果的临时下载地址。文件类型为zip压缩包，下载链接有效期30分钟。压缩包内包含*.md、*.json以及images文件夹。
    ...
    /**
     * @var integer 文档解析失败页数
     */
    public $FailPageNum;

    /**
     * @var integer 文档解析成功页数
     */
     ...
     * @param string $DocumentRecognizeResultUrl 文档解析结果的临时下载地址。文件类型为zip压缩包，下载链接有效期30分钟。压缩包内包含*.md、*.json以及images文件夹。

     * @param array $FailedPages 文档解析失败的页码。
     * @param integer $FailPageNum 文档解析失败页数
     * @param integer $SuccessPageNum 文档解析成功页数
