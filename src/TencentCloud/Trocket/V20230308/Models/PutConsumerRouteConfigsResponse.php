 ...
 * @method array getFailures() 获取<p>写入失败的消费者路由配置列表</p>
 * @method void setFailures(array $Failures) 设置<p>写入失败的消费者路由配置列表</p>
 * @method string getRequestId() 获取唯一请求 ID，由服务端生成，每次请求都会返回（若请求因其他原因未能抵达服务端，则该次请求不会获得 RequestId）。定位问题时需要提供该次请求的 RequestId。
 * @method void setRequestId(string $RequestId) 设置唯一请求 ID，由服务端生成，每次请求都会返回（若请求因其他原因未能抵达服务端，则该次请求不会获得 RequestId）。定位问题时需要提供该次请求的 RequestId。
 */
class PutConsumerRouteConfigsResponse extends AbstractModel
{
    /**
     * @var integer 查询总数
     */
     ...
     * @param array $Failures <p>写入失败的消费者路由配置列表</p>
    ...
    public function deserialize($param)
    {
        ...
        if (array_key_exists("Failures",$param) and $param["Failures"] !== null) {
            ...
            foreach ($param["Failures"] as $key => $value){
                $obj = new PutConsumerRouteConfigFailure();
