 ...
 * @method string getResultUrl() 获取RspImgType 为 url 时，返回处理后的图片 url 数据。(暂时不支持)
 * @method void setResultUrl(string $ResultUrl) 设置RspImgType 为 url 时，返回处理后的图片 url 数据。(暂时不支持)
...
class FaceCartoonPicResponse extends AbstractModel
{
    ...
    /**
     * @var string RspImgType 为 url 时，返回处理后的图片 url 数据。(暂时不支持)
     */
    public $ResultUrl;
     ...
     * @param string $ResultUrl RspImgType 为 url 时，返回处理后的图片 url 数据。(暂时不支持)
