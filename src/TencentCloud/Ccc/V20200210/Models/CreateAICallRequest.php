 ...
 * @method string getSystemPrompt() 获取用于设定AI人设、说话规则、任务等的全局提示词。
 ...
 * @method void setSystemPrompt(string $SystemPrompt) 设置用于设定AI人设、说话规则、任务等的全局提示词。
 ...
 * @method string getLLMType() 获取模型接口协议类型，目前兼容三种协议类型：

- OpenAI协议(包括GPT、混元、DeepSeek等)："openai"
- Azure协议："azure"
- Minimax协议："minimax"
 * @method void setLLMType(string $LLMType) 设置模型接口协议类型，目前兼容三种协议类型：
...
class CreateAICallRequest extends AbstractModel
{
    ...
    /**
     * @var string 用于设定AI人设、说话规则、任务等的全局提示词。
...
8.询问家长是否有问题需要咨询，如果没有请礼貌道别并用call_end挂断电话。
    ...
    /**
     * @var string 模型接口协议类型，目前兼容三种协议类型：
...
```
     ...
     * @param string $SystemPrompt 用于设定AI人设、说话规则、任务等的全局提示词。
...
8.询问家长是否有问题需要咨询，如果没有请礼貌道别并用call_end挂断电话。
     ...
     * @param string $LLMType 模型接口协议类型，目前兼容三种协议类型：
