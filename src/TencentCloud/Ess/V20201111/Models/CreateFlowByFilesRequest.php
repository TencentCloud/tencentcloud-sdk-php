...
如果合同流程是有序签署，Approvers列表中参与人的顺序就是默认的签署顺序，请确保列表中参与人的顺序符合实际签署顺序。
 * @method array getFileIds() 获取本合同流程需包含的PDF文件资源编号列表，通过UploadFiles接口获取PDF文件资源编号。目前，此接口仅支持单个文件发起。
 * @method void setFileIds(array $FileIds) 设置本合同流程需包含的PDF文件资源编号列表，通过UploadFiles接口获取PDF文件资源编号。目前，此接口仅支持单个文件发起。
...
true：预览模式，不产生合同流程，不返回合同流程编号FlowId，而是返回预览链接PreviewUrl，有效期为300秒，用于查看真实发起后合同的样子。
 * @method integer getPreviewType() 获取
预览模式下产生的预览链接类型 
 ...
 * @method void setPreviewType(integer $PreviewType) 设置
预览模式下产生的预览链接类型 
...
到达提醒时间后，腾讯电子签会短信通知发起方企业合同提醒，可用于处理合同到期事务，如合同续签等事宜。
 * @method boolean getUnordered() 获取
合同流程的签署顺序类型：
- false：(默认)有序签署, 本合同多个参与人需要依次签署
- true：无序签署, 本合同多个参与人没有先后签署限制
 * @method void setUnordered(boolean $Unordered) 设置
合同流程的签署顺序类型：
- false：(默认)有序签署, 本合同多个参与人需要依次签署
- true：无序签署, 本合同多个参与人没有先后签署限制
...
签署方：李四
```
 * @method boolean getNeedSignReview() 获取
 ...
 * @method void setNeedSignReview(boolean $NeedSignReview) 设置
发起方企业的签署人进行签署操作前，是否需要企业内部走审批流程，取值如下：
...
如果合同流程是有序签署，Approvers列表中参与人的顺序就是默认的签署顺序，请确保列表中参与人的顺序符合实际签署顺序。
    ...
    /**
     * @var array 本合同流程需包含的PDF文件资源编号列表，通过UploadFiles接口获取PDF文件资源编号。目前，此接口仅支持单个文件发起。
     */
    public $FileIds;
...
如果合同流程是有序签署，Approvers列表中参与人的顺序就是默认的签署顺序，请确保列表中参与人的顺序符合实际签署顺序。
     * @param array $FileIds 本合同流程需包含的PDF文件资源编号列表，通过UploadFiles接口获取PDF文件资源编号。目前，此接口仅支持单个文件发起。
