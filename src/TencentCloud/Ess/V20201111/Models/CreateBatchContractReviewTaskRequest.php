 ...
 * @method void setPolicyType(integer $PolicyType) 设置合同审查的审查尺度。默认为`0`严格尺度
...
在集团企业代理子企业操作的场景中，需设置此参数。在此情境下，ProxyOrganizationId（子企业的组织ID）为必填项。
 * @method string getComment() 获取备注信息，长度不能超过100个字符
 * @method void setComment(string $Comment) 设置备注信息，长度不能超过100个字符
...
注:  `目前，此接口仅支持5个文件发起。每个文件限制在10M以下，文件必须是PDF格式`
    ...
    /**
     * @var integer 合同审查的审查尺度。默认为`0`严格尺度
