...
- 默认值为1（仅检测图片中面积最大的那张人脸），最大值为120。 
- 此参数用于控制处理待检测图片中的人脸个数，值越小，处理速度越快。
 * @method integer getMinFaceSize() 获取人脸长和宽的最小尺寸，单位为像素，低于MinFaceSize值的人脸不会被检测。
- 只支持设置34和20，建议使用34。
 * @method void setMinFaceSize(integer $MinFaceSize) 设置人脸长和宽的最小尺寸，单位为像素，低于MinFaceSize值的人脸不会被检测。
- 只支持设置34和20，建议使用34。
...
- 非腾讯云存储的Url速度和稳定性可能受一定影响。
- 支持PNG、JPG、JPEG、BMP，不支持 GIF 图片。
 * @method integer getNeedFaceAttributes() 获取是否需要返回人脸属性信息（FaceAttributesInfo）。
- 0 为不需要返回，1 为需要返回。
- 默认为 0。 
 ...
 * @method void setNeedFaceAttributes(integer $NeedFaceAttributes) 设置是否需要返回人脸属性信息（FaceAttributesInfo）。
- 0 为不需要返回，1 为需要返回。
- 默认为 0。 
- 非 1 值均视为不需要返回，此时 FaceAttributesInfo 不具备参考意义。  
- 最多返回面积最大的 5 张人脸属性信息，超过 5 张人脸（第 6 张及以后的人脸）的 FaceAttributesInfo 不具备参考意义。  
- 提取人脸属性信息较为耗时，如不需要人脸属性信息，建议关闭此项功能，加快人脸检测速度。
 * @method integer getNeedQualityDetection() 获取是否开启质量检测。
- 0 为关闭，1 为开启。
- 默认为 0。 
- 非 1 值均视为不进行质量检测。
- 最多返回面积最大的 30 张人脸质量分信息，超过 30 张人脸（第 31 张及以后的人脸）的 FaceQualityInfo不具备参考意义。  
- 建议：人脸入库操作建议开启此功能。
 * @method void setNeedQualityDetection(integer $NeedQualityDetection) 设置是否开启质量检测。
- 0 为关闭，1 为开启。
- 默认为 0。 
- 非 1 值均视为不进行质量检测。
- 最多返回面积最大的 30 张人脸质量分信息，超过 30 张人脸（第 31 张及以后的人脸）的 FaceQualityInfo不具备参考意义。  
...
- 此参数用于控制处理待检测图片中的人脸个数，值越小，处理速度越快。
    ...
    /**
     * @var integer 人脸长和宽的最小尺寸，单位为像素，低于MinFaceSize值的人脸不会被检测。
- 只支持设置34和20，建议使用34。
     */
    public $MinFaceSize;
...
- 支持PNG、JPG、JPEG、BMP，不支持 GIF 图片。
    ...
    /**
     * @var integer 是否需要返回人脸属性信息（FaceAttributesInfo）。
- 0 为不需要返回，1 为需要返回。
- 默认为 0。 
- 非 1 值均视为不需要返回，此时 FaceAttributesInfo 不具备参考意义。  
- 最多返回面积最大的 5 张人脸属性信息，超过 5 张人脸（第 6 张及以后的人脸）的 FaceAttributesInfo 不具备参考意义。  
...
- 默认值为1（仅检测图片中面积最大的那张人脸），最大值为120。 
- 此参数用于控制处理待检测图片中的人脸个数，值越小，处理速度越快。
     * @param integer $MinFaceSize 人脸长和宽的最小尺寸，单位为像素，低于MinFaceSize值的人脸不会被检测。
- 只支持设置34和20，建议使用34。
...
- 非腾讯云存储的Url速度和稳定性可能受一定影响。
- 支持PNG、JPG、JPEG、BMP，不支持 GIF 图片。
     * @param integer $NeedFaceAttributes 是否需要返回人脸属性信息（FaceAttributesInfo）。
- 0 为不需要返回，1 为需要返回。
- 默认为 0。 
- 非 1 值均视为不需要返回，此时 FaceAttributesInfo 不具备参考意义。  
- 最多返回面积最大的 5 张人脸属性信息，超过 5 张人脸（第 6 张及以后的人脸）的 FaceAttributesInfo 不具备参考意义。  
- 提取人脸属性信息较为耗时，如不需要人脸属性信息，建议关闭此项功能，加快人脸检测速度。
     * @param integer $NeedQualityDetection 是否开启质量检测。
- 0 为关闭，1 为开启。
- 默认为 0。 
- 非 1 值均视为不进行质量检测。
- 最多返回面积最大的 30 张人脸质量分信息，超过 30 张人脸（第 31 张及以后的人脸）的 FaceQualityInfo不具备参考意义。  
