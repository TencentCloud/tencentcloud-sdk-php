 ...
 * @method array getConfigs() 获取<p>批量写入的消费者路由配置列表</p><p>入参限制：批量上限为 32 条</p><p>覆盖式写入</p>
 * @method void setConfigs(array $Configs) 设置<p>批量写入的消费者路由配置列表</p><p>入参限制：批量上限为 32 条</p><p>覆盖式写入</p>
 */
class PutConsumerRouteConfigsRequest extends AbstractModel
{
    /**
    ...
    /**
     * @var array <p>批量写入的消费者路由配置列表</p><p>入参限制：批量上限为 32 条</p><p>覆盖式写入</p>
     */
    public $Configs;
     ...
     * @param array $Configs <p>批量写入的消费者路由配置列表</p><p>入参限制：批量上限为 32 条</p><p>覆盖式写入</p>
     */
    ...
    public function deserialize($param)
    {
        ...
        if (array_key_exists("Configs",$param) and $param["Configs"] !== null) {
            ...
            foreach ($param["Configs"] as $key => $value){
                $obj = new PutConsumerRouteConfigItem();
