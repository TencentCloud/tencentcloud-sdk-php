 ...
 * @method Models\CreateCloudTranscriptionResponse CreateCloudTranscription(Models\CreateCloudTranscriptionRequest $req) 接口说明：
启动云端转录功能。
 * @method Models\CreateLiveStreamModerationResponse CreateLiveStreamModeration(Models\CreateLiveStreamModerationRequest $req) 启动一路直播流审核。服务端异步拉流、定频截帧、音频切片、送审，通过回调返回结果。一次一个任务（一路流）。您可以通过此接口实现如下目标：
 ...
 * @method Models\DescribeCloudModerationResponse DescribeCloudModeration(Models\DescribeCloudModerationRequest $req) 成功开启AI 内容理解任务后，可以使用此接口来查询AI 内容理解任务状态和订阅的黑白名单信息。仅在任务进行时有效，任务退出后查询将会返回错误。
 ...
 * @method Models\DescribeCloudTranscriptionResponse DescribeCloudTranscription(Models\DescribeCloudTranscriptionRequest $req) 成功开启转录后，可以使用此接口来查询录制状态。仅在转录任务进行时有效，转录退出后查询将会返回错误。
 * @method Models\DescribeLiveStreamModerationResponse DescribeLiveStreamModeration(Models\DescribeLiveStreamModerationRequest $req) 成功开启直播流AI 内容理解任务后，可以使用此接口来查询AI 内容理解任务状态，仅在任务进行时有效，任务退出后查询将会返回错误。
 ...
 * @method Models\DescribeTRTCAIRecognitionUsageResponse DescribeTRTCAIRecognitionUsage(Models\DescribeTRTCAIRecognitionUsageRequest $req) AI 智能识别与对话用量查询（AI对话/语音转文本/实时翻译/实时语音合成）
- 查询时间小于等于1天时，返回每5分钟粒度的数据；查询时间大于1天时，返回按天汇总的数据。
- 单次查询统计区间最多不能超过31天。
- 若查询当天用量，由于统计延迟等原因，返回数据可能不够准确。
 * @method Models\DescribeTRTCDedicatedCloudAccUsageResponse DescribeTRTCDedicatedCloudAccUsage(Models\DescribeTRTCDedicatedCloudAccUsageRequest $req) TRTC专属云网络加速用量查询
- 查询时间小于等于1天时，返回每5分钟粒度的数据；查询时间大于1天时，返回按天汇总的数据。
- 单次查询统计区间最多不能超过31天。
- 若查询当天用量，由于统计延迟等原因，返回数据可能不够准确。
...
1.调用接口需开通监控仪表盘【基础版】和【进阶版】，监控仪表盘【免费版】不支持调用，监控仪表盘[版本功能和计费说明](https://cloud.tencent.com/document/product/647/81331)。
2.查询时间范围根据监控仪表盘功能版本而定，【基础版】可查近30天，【进阶版】可查近60天。
 * @method Models\DescribeTRTCMarketScaleMetricDataResponse DescribeTRTCMarketScaleMetricData(Models\DescribeTRTCMarketScaleMetricDataRequest $req) 云监控monitor接口已下线，trtc同步下线接口
...
1.调用接口需开通监控仪表盘【基础版】和【进阶版】，监控仪表盘【免费版】不支持调用，详情参考[监控仪表盘](https://cloud.tencent.com/document/product/647/81331)。
2.查询时间范围根据监控仪表盘功能版本而定，基础版可查近3小时，进阶版可查近12小时。
 * @method Models\DescribeTRTCSegmentModerationUsageResponse DescribeTRTCSegmentModerationUsage(Models\DescribeTRTCSegmentModerationUsageRequest $req) 切片截图与内容理解用量查询，支持查询音视频切片（云端切片场景）和 AI 内容理解（审核场景）两种业务类型
- 查询时间小于等于1天时，返回每5分钟粒度的数据；查询时间大于1天时，返回按天汇总的数据。
- 单次查询统计区间最多不能超过31天。
- 若查询当天用量，由于统计延迟等原因，返回数据可能不够准确。
