 ...
 * @method integer getMemoryQuota() 获取内存配额（单位 MB）
 * @method void setMemoryQuota(integer $MemoryQuota) 设置内存配额（单位 MB）
 */
class ModifyAndroidInstancesResourcesRequest extends AbstractModel
{
    /**
    ...
    /**
     * @var integer 内存配额（单位 MB）
     */
    public $MemoryQuota;

    /**
     * @param array $AndroidInstanceIds 安卓实例 ID 列表（最大100条数据）
     * @param integer $MemoryQuota 内存配额（单位 MB）
     */
    ...
    public function deserialize($param)
    {
        ...
        if (array_key_exists("MemoryQuota",$param) and $param["MemoryQuota"] !== null) {
            $this->MemoryQuota = $param["MemoryQuota"];
        }
    }
}
