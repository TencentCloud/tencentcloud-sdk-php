...
域名+AppName+StreamName唯一标识单个转码规则，如需删除需要强匹配，例如AppName为空也需要传空字符串进行强匹配。
 * @method integer getTemplateId() 获取直播垫片模板id。
 * @method void setTemplateId(integer $TemplateId) 设置直播垫片模板id。
 */
class DeleteLivePadRuleRequest extends AbstractModel
{
    /**
     * @var string 推流域名。
域名+AppName+StreamName唯一标识单个转码规则，如需删除需要强匹配，例如AppName为空也需要传空字符串进行强匹配。
...
域名+AppName+StreamName唯一标识单个转码规则，如需删除需要强匹配，例如AppName为空也需要传空字符串进行强匹配。
     * @param integer $TemplateId 直播垫片模板id。
     */
