<?php
/*
 * Copyright (c) 2017-2025 Tencent. All Rights Reserved.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *    http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
namespace TencentCloud\Adp\V20260520\Models;
use TencentCloud\Common\AbstractModel;

/**
 * DescribeDoc请求参数结构体
 *
 * @method string getDocId() 获取<p>文档 ID</p>
 * @method void setDocId(string $DocId) 设置<p>文档 ID</p>
 * @method string getKbId() 获取<p>所属知识库 ID</p>
 * @method void setKbId(string $KbId) 设置<p>所属知识库 ID</p>
 * @method FieldMask getReadMask() 获取<p>字段掩码：当前支持的 Path：DocLink.CosUrl，其他未列举的字段默认都返回</p>
 * @method void setReadMask(FieldMask $ReadMask) 设置<p>字段掩码：当前支持的 Path：DocLink.CosUrl，其他未列举的字段默认都返回</p>
 */
class DescribeDocRequest extends AbstractModel
{
    /**
     * @var string <p>文档 ID</p>
     */
    public $DocId;

    /**
     * @var string <p>所属知识库 ID</p>
     */
    public $KbId;

    /**
     * @var FieldMask <p>字段掩码：当前支持的 Path：DocLink.CosUrl，其他未列举的字段默认都返回</p>
     */
    public $ReadMask;

    /**
     * @param string $DocId <p>文档 ID</p>
     * @param string $KbId <p>所属知识库 ID</p>
     * @param FieldMask $ReadMask <p>字段掩码：当前支持的 Path：DocLink.CosUrl，其他未列举的字段默认都返回</p>
     */
    function __construct()
    {

    }

    /**
     * For internal only. DO NOT USE IT.
     */
    public function deserialize($param)
    {
        if ($param === null) {
            return;
        }
        if (array_key_exists("DocId",$param) and $param["DocId"] !== null) {
            $this->DocId = $param["DocId"];
        }

        if (array_key_exists("KbId",$param) and $param["KbId"] !== null) {
            $this->KbId = $param["KbId"];
        }

        if (array_key_exists("ReadMask",$param) and $param["ReadMask"] !== null) {
            $this->ReadMask = new FieldMask();
            $this->ReadMask->deserialize($param["ReadMask"]);
        }
    }
}
