...
class UpdateMessageEnrichmentRulePriorityRequest extends AbstractModel
{
    ...
    public function deserialize($param)
    {
        ...
        if (array_key_exists("Priorities",$param) and $param["Priorities"] !== null) {
            $this->Priorities = [];
            foreach ($param["Priorities"] as $key => $value){
                $obj = new MessageEnrichmentRulePriority();
                $obj->deserialize($value);
                array_push($this->Priorities, $obj);
            }
        }
    }
}
