 ...
 * @method string getCertificatePrivateKey() 获取证书私钥，若上传证书公钥， 则证书私钥必填
 * @method void setCertificatePrivateKey(string $CertificatePrivateKey) 设置证书私钥，若上传证书公钥， 则证书私钥必填
 * @method integer getExpiringNotificationSwitch() 获取旧证书是否忽略到期提醒  0:不忽略通知。1:忽略通知
 * @method void setExpiringNotificationSwitch(integer $ExpiringNotificationSwitch) 设置旧证书是否忽略到期提醒  0:不忽略通知。1:忽略通知
 * @method boolean getRepeatable() 获取相同的证书是否允许重复上传，若上传证书公钥， 则可以配置该参数
 * @method void setRepeatable(boolean $Repeatable) 设置相同的证书是否允许重复上传，若上传证书公钥， 则可以配置该参数
 * @method boolean getAllowDownload() 获取是否允许下载，若上传证书公钥， 则可以配置该参数
 * @method void setAllowDownload(boolean $AllowDownload) 设置是否允许下载，若上传证书公钥， 则可以配置该参数
 * @method array getTags() 获取标签列表，若上传证书公钥， 则可以配置该参数
 * @method void setTags(array $Tags) 设置标签列表，若上传证书公钥， 则可以配置该参数
 * @method integer getProjectId() 获取项目 ID，若上传证书公钥， 则可以配置该参数
 * @method void setProjectId(integer $ProjectId) 设置项目 ID，若上传证书公钥， 则可以配置该参数
 */
class UpdateCertificateInstanceRequest extends AbstractModel
{
    ...
    /**
     * @var string 证书私钥，若上传证书公钥， 则证书私钥必填
     */
    public $CertificatePrivateKey;

    /**
     * @var integer 旧证书是否忽略到期提醒  0:不忽略通知。1:忽略通知
     */
    public $ExpiringNotificationSwitch;

    /**
     * @var boolean 相同的证书是否允许重复上传，若上传证书公钥， 则可以配置该参数
     */
    public $Repeatable;

    /**
     * @var boolean 是否允许下载，若上传证书公钥， 则可以配置该参数
     */
    public $AllowDownload;

    /**
     * @var array 标签列表，若上传证书公钥， 则可以配置该参数
     */
    public $Tags;

    /**
     * @var integer 项目 ID，若上传证书公钥， 则可以配置该参数
     */
    public $ProjectId;
     ...
     * @param string $CertificatePrivateKey 证书私钥，若上传证书公钥， 则证书私钥必填
     * @param integer $ExpiringNotificationSwitch 旧证书是否忽略到期提醒  0:不忽略通知。1:忽略通知
     * @param boolean $Repeatable 相同的证书是否允许重复上传，若上传证书公钥， 则可以配置该参数
     * @param boolean $AllowDownload 是否允许下载，若上传证书公钥， 则可以配置该参数
     * @param array $Tags 标签列表，若上传证书公钥， 则可以配置该参数
     * @param integer $ProjectId 项目 ID，若上传证书公钥， 则可以配置该参数
     */
