<?php
/*
 * Copyright (c) 2017-2018 THL A29 Limited, a Tencent company. All Rights Reserved.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *    http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
namespace TencentCloud\Lke\V20231130\Models;
use TencentCloud\Common\AbstractModel;

/**
 * 相似问修改(更新)信息
 *
 * @method array getAddQuestions() 获取需要添加的相似问(内容)列表
 * @method void setAddQuestions(array $AddQuestions) 设置需要添加的相似问(内容)列表
 * @method array getUpdateQuestions() 获取需要更新的相似问列表
 * @method void setUpdateQuestions(array $UpdateQuestions) 设置需要更新的相似问列表
 * @method array getDeleteQuestions() 获取需要删除的相似问列表
 * @method void setDeleteQuestions(array $DeleteQuestions) 设置需要删除的相似问列表
 */
class SimilarQuestionModify extends AbstractModel
{
    /**
     * @var array 需要添加的相似问(内容)列表
     */
    public $AddQuestions;

    /**
     * @var array 需要更新的相似问列表
     */
    public $UpdateQuestions;

    /**
     * @var array 需要删除的相似问列表
     */
    public $DeleteQuestions;

    /**
     * @param array $AddQuestions 需要添加的相似问(内容)列表
     * @param array $UpdateQuestions 需要更新的相似问列表
     * @param array $DeleteQuestions 需要删除的相似问列表
     */
    function __construct()
    {

    }

    /**
     * For internal only. DO NOT USE IT.
     */
    public function deserialize($param)
    {
        if ($param === null) {
            return;
        }
        if (array_key_exists("AddQuestions",$param) and $param["AddQuestions"] !== null) {
            $this->AddQuestions = $param["AddQuestions"];
        }

        if (array_key_exists("UpdateQuestions",$param) and $param["UpdateQuestions"] !== null) {
            $this->UpdateQuestions = [];
            foreach ($param["UpdateQuestions"] as $key => $value){
                $obj = new SimilarQuestion();
                $obj->deserialize($value);
                array_push($this->UpdateQuestions, $obj);
            }
        }

        if (array_key_exists("DeleteQuestions",$param) and $param["DeleteQuestions"] !== null) {
            $this->DeleteQuestions = [];
            foreach ($param["DeleteQuestions"] as $key => $value){
                $obj = new SimilarQuestion();
                $obj->deserialize($value);
                array_push($this->DeleteQuestions, $obj);
            }
        }
    }
}
