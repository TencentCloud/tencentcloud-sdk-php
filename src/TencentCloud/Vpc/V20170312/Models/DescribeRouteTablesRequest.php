 ...
 * @method array getRouteTableIds() 获取<p>路由表实例ID，例如：rtb-azd4dt1c。</p>
 * @method void setRouteTableIds(array $RouteTableIds) 设置<p>路由表实例ID，例如：rtb-azd4dt1c。</p>
 * @method string getOffset() 获取<p>偏移量。</p>
 * @method void setOffset(string $Offset) 设置<p>偏移量。</p>
 * @method string getLimit() 获取<p>返回数量，默认为20，最大值为100。</p>
 * @method void setLimit(string $Limit) 设置<p>返回数量，默认为20，最大值为100。</p>
 * @method boolean getNeedRouterInfo() 获取<p>是否需要获取路由策略信息，默认获取，当控制台不需要拉取路由策略信息时，改为False。</p>
 * @method void setNeedRouterInfo(boolean $NeedRouterInfo) 设置<p>是否需要获取路由策略信息，默认获取，当控制台不需要拉取路由策略信息时，改为False。</p>
 */
class DescribeRouteTablesRequest extends AbstractModel
{
    /**
     * @var array <p>过滤条件，参数不支持同时指定RouteTableIds和Filters。</p><li>route-table-id - String - （过滤条件）路由表实例ID。</li><li>route-table-name - String - （过滤条件）路由表名称。</li><li>vpc-id - String - （过滤条件）VPC实例ID，形如：vpc-f49l6u0z。</li><li>association.main - String - （过滤条件）是否主路由表。</li><li>tag-key - String -是否必填：否 - （过滤条件）按照标签键进行过滤。</li><li>tag:tag-key - String - 是否必填：否 - （过滤条件）按照标签键值对进行过滤。 tag-key使用具体的标签键进行替换。使用请参考示例2。</li><li>visible - String - （过滤条件）是否可见。</li><li>next-hop-type - String - 是否必填：否 - （过滤条件）按下一跳类型进行过滤。使用next-hop-type进行过滤时，必须同时携带route-table-id与vpc-id。目前我们支持的类型有：LOCAL: 本地路由CVM：公网网关类型的云服务器；VPN：VPN网关；DIRECTCONNECT：专线网关；PEERCONNECTION：对等连接；HAVIP：高可用虚拟IP；NAT：NAT网关; NORMAL_CVM：普通云服务器；EIP：云服务器的公网IP；CCN：云联网；LOCAL_GATEWAY：本地网关。GWLB_ENDPOINT：网关负载均衡终端节点。</li>
     */
    public $Filters;

    /**
     * @var array <p>路由表实例ID，例如：rtb-azd4dt1c。</p>
     */
    ...
    /**
     * @var boolean <p>是否需要获取路由策略信息，默认获取，当控制台不需要拉取路由策略信息时，改为False。</p>
     */
     ...
     * @param array $RouteTableIds <p>路由表实例ID，例如：rtb-azd4dt1c。</p>
     * @param string $Offset <p>偏移量。</p>
     * @param string $Limit <p>返回数量，默认为20，最大值为100。</p>
     * @param boolean $NeedRouterInfo <p>是否需要获取路由策略信息，默认获取，当控制台不需要拉取路由策略信息时，改为False。</p>
     */
