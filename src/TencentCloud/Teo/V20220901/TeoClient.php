...
/**
 * @method Models\ApplyFreeCertificateResponse ApplyFreeCertificate(Models\ApplyFreeCertificateRequest $req) 申请免费证书时，如果您需要通过使用 DNS 委派验证或者文件验证进行申请，您可以调用该接口来进行发起证书申请并根据申请方式来获取对应的验证内容。调用接口的顺序如下：
 ...
 * @method Models\CheckCnameStatusResponse CheckCnameStatus(Models\CheckCnameStatusRequest $req) 当站点接入类型为 CNAME 接入类型时，要求该站点下的所有接入域名必须按照 EdgeOne 分配的指定 CNAME 域名完成 CNAME 记录配置。
 ...
 * @method Models\CreateLogAnalysisDownloadTaskResponse CreateLogAnalysisDownloadTask(Models\CreateLogAnalysisDownloadTaskRequest $req) 本接口用以创建日志分析下载任务，创建完成后可通过 DescribeLogAnalysisDownloadTasks 接口查询下载任务。
注意：
1.单次最多支持下载 5000万条日志。
2.日志文件将保留 3 天。
 ...
 * @method Models\DeleteFunctionReplicaResponse DeleteFunctionReplica(Models\DeleteFunctionReplicaRequest $req) 本接口用于删除指定的边缘函数副本。
 * @method Models\DeleteFunctionRulesResponse DeleteFunctionRules(Models\DeleteFunctionRulesRequest $req) 删除边缘函数触发规则。
 * @method Models\DeleteInferenceAPITokenResponse DeleteInferenceAPIToken(Models\DeleteInferenceAPITokenRequest $req) 删除推理 API Token，删除后该 Token 立即失效，使用其访问推理服务的请求将无法通过鉴权。
 ...
 * @method Models\DescribeEdgeKVNamespacesResponse DescribeEdgeKVNamespaces(Models\DescribeEdgeKVNamespacesRequest $req) 查询指定站点下的 KV 命名空间列表，支持分页、排序和条件过滤。返回命名空间的基本信息、存储容量使用情况以及被引用关系。若查询不到数据，则返回空数组。
 ...
 * @method Models\DescribeFunctionComponentBindingsResponse DescribeFunctionComponentBindings(Models\DescribeFunctionComponentBindingsRequest $req) 本接口用于查询指定边缘函数的组件绑定列表，支持分页和条件过滤，返回绑定的组件类型、变量名及配置参数等详细信息。当前支持的绑定组件类型为 KV 命名空间（kv_namespace）。
 * @method Models\DescribeFunctionReplicasResponse DescribeFunctionReplicas(Models\DescribeFunctionReplicasRequest $req) 本接口用于查询边缘函数的副本列表。
 ...
 * @method Models\DescribeInferenceServiceMonitorDataResponse DescribeInferenceServiceMonitorData(Models\DescribeInferenceServiceMonitorDataRequest $req) 查询推理服务监控数据，支持 CPU、内存、GPU、显存使用率和实例数量等指标，可指定时间范围和聚合粒度，最多查询最近 30 天的数据。
 * @method Models\DescribeInferenceServicesResponse DescribeInferenceServices(Models\DescribeInferenceServicesRequest $req) 查询推理服务列表，支持按服务名称、服务 ID、状态过滤，返回服务的配置、运行状态、实例数和推理访问地址等信息。
 ...
 * @method Models\DescribeLogAnalysisDetailResponse DescribeLogAnalysisDetail(Models\DescribeLogAnalysisDetailRequest $req) 本接口用以查询日志分析日志详情，数据来自站点下实时日志推送任务目的地为 "log-analysis" 的任务推送的日志数据。
 * @method Models\DescribeLogAnalysisDownloadTasksResponse DescribeLogAnalysisDownloadTasks(Models\DescribeLogAnalysisDownloadTasksRequest $req) 本接口用以查询日志分析日志下载任务列表。注意：只保留最近三天的下载任务记录。
 ...
 * @method Models\DescribePrefetchOriginLimitResponse DescribePrefetchOriginLimit(Models\DescribePrefetchOriginLimitRequest $req) 本接口用于查询回源限速限制，该功能白名单内测中。
 ...
 * @method Models\DescribeSharedCNAMEResponse DescribeSharedCNAME(Models\DescribeSharedCNAMERequest $req) 查询共享CNAME列表，支持模糊搜索、分页、排序等。
 ...
 * @method Models\EdgeKVPutResponse EdgeKVPut(Models\EdgeKVPutRequest $req) 本接口用于向指定命名空间写入键值对数据，支持设置过期时间。若键已存在则覆盖原有值，若不存在则创建新键值对。
 ...
 * @method Models\ExportZoneConfigResponse ExportZoneConfig(Models\ExportZoneConfigRequest $req) 导出站点配置接口，本接口支持用户根据需要的配置项进行配置导出，导出的配置用于导入站点配置接口（ImportZoneConfig）进行配置导入。
 ...
 * @method Models\ImportZoneConfigResponse ImportZoneConfig(Models\ImportZoneConfigRequest $req) 导入站点配置接口，本接口支持站点配置文件的快速导入，发起导入后接口会返回对应的任务 ID（TaskId），用户需通过查询站点配置导入结果接口（DescribeZoneConfigImportResult）获取本次导入任务执行的结果。
 ...
 * @method Models\ModifyEdgeKVNamespaceResponse ModifyEdgeKVNamespace(Models\ModifyEdgeKVNamespaceRequest $req) 本接口用于修改指定 KV 命名空间的属性信息，当前支持修改命名空间描述。
 ...
 * @method Models\ModifyFunctionComponentBindingsResponse ModifyFunctionComponentBindings(Models\ModifyFunctionComponentBindingsRequest $req) 修改边缘函数与组件的绑定关系，支持绑定（bind）、覆盖绑定（bind-override）、解绑（unbind）和重置绑定（rebind）四种操作模式。通过指定操作类型和组件列表，可实现对函数组件绑定关系的管理。
 * @method Models\ModifyFunctionReplicaResponse ModifyFunctionReplica(Models\ModifyFunctionReplicaRequest $req) 本接口用于修改指定边缘函数副本的内容和描述。
 ...
 * @method Models\ModifyInferenceServiceResponse ModifyInferenceService(Models\ModifyInferenceServiceRequest $req) 修改推理服务，支持更新监听端口、请求路径、容器镜像、资源配置和描述信息，仅传入的参数会被修改，未传入的参数保持不变。
 ...
 * @method Models\ModifyOriginACLResponse ModifyOriginACL(Models\ModifyOriginACLRequest $req) 本接口用于对七层加速域名/四层代理实例启用/关闭特定回源 IP 网段回源。单次支持提交的七层加速域名的数量最大为 200，四层代理实例的数量最大为 100，支持七层加速域名/四层代理实例混合提交，总实例个数最大为 200。如需变更超过 200 个实例，请通过本接口分批提交。同时对于开白的客户支持切换到其他可用的源站防护 IP 网段版本，例如精简版，可以减少回源 IP 网段。
 ...
 * @method Models\ModifyPrefetchOriginLimitResponse ModifyPrefetchOriginLimit(Models\ModifyPrefetchOriginLimitRequest $req) 本接口用于配置回源限速限制，该功能白名单内测中。
