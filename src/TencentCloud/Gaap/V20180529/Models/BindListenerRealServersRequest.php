 ...
 * @method array getRealServerBindSet() 获取<p>待绑定源站列表。如果该监听器的源站调度策略是加权轮询，需要填写源站权重 RealServerWeight, 不填或者其他调度类型默认源站权重为1。</p>
 * @method void setRealServerBindSet(array $RealServerBindSet) 设置<p>待绑定源站列表。如果该监听器的源站调度策略是加权轮询，需要填写源站权重 RealServerWeight, 不填或者其他调度类型默认源站权重为1。</p>
 */
class BindListenerRealServersRequest extends AbstractModel
{
    ...
    /**
     * @var array <p>待绑定源站列表。如果该监听器的源站调度策略是加权轮询，需要填写源站权重 RealServerWeight, 不填或者其他调度类型默认源站权重为1。</p>
     */
    public $RealServerBindSet;

    /**
     * @param string $ListenerId <p>监听器ID</p>
     * @param array $RealServerBindSet <p>待绑定源站列表。如果该监听器的源站调度策略是加权轮询，需要填写源站权重 RealServerWeight, 不填或者其他调度类型默认源站权重为1。</p>
     */
