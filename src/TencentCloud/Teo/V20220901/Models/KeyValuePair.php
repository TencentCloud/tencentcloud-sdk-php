 ...
 * @method void setExpiration(string $Expiration) 设置过期时间，遵循 ISO 8601 标准，格式为 YYYY-MM-DDThh:mm:ssZ（UTC 时间）。出参时若为空字符串，表示该键值对永不过期。
 */
class KeyValuePair extends AbstractModel
{
    ...
    /**
     * @var string 键值。入参时不能为空，最大支持 1 MB。出参时若键不存在，则返回空字符串。
     */
    public $Value;

    /**
     * @var string 过期时间，遵循 ISO 8601 标准，格式为 YYYY-MM-DDThh:mm:ssZ（UTC 时间）。出参时若为空字符串，表示该键值对永不过期。
     */
    public $Expiration;
