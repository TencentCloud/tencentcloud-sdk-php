 ...
 * @method string getWarnings() 获取告警信息。
 ...
 * @method void setWarnings(string $Warnings) 设置告警信息。
- 当在Config中配置了告警信息会停止人像比对，Result返回错误（FailedOperation.OcrWarningOccurred）并有此告警信息。
- Code 告警码列表和释义：
'-9101'：身份证边框不完整告警。
'-9102'：身份证复印件告警。
...
- 多个会用“|” 隔开，如 "-9101|-9106|-9104"。
 * @method float getQuality() 获取图片质量分数。
- 当请求Config中配置图片模糊告警该参数才有意义。
- 取值范围（0～100），目前默认阈值是50分，低于50分会触发模糊告警。
 * @method void setQuality(float $Quality) 设置图片质量分数。
- 当请求Config中配置图片模糊告警该参数才有意义。
- 取值范围（0～100），目前默认阈值是50分，低于50分会触发模糊告警。
