 ...
 * @method string getAlarmRuleId() 获取告警规则唯一id，通过接口GetAlarmRule/ListAlarmRule获取
 * @method void setAlarmRuleId(string $AlarmRuleId) 设置告警规则唯一id，通过接口GetAlarmRule/ListAlarmRule获取
 * @method string getAlarmRuleName() 获取告警规则新的规则名称
 * @method void setAlarmRuleName(string $AlarmRuleName) 设置告警规则新的规则名称
 ...
 * @method integer getStatus() 获取告警规则的启用状态0--禁用1--启用
 * @method void setStatus(integer $Status) 设置告警规则的启用状态0--禁用1--启用
 ...
 * @method string getDescription() 获取告警描述信息
 * @method void setDescription(string $Description) 设置告警描述信息
 */
class UpdateOpsAlarmRuleRequest extends AbstractModel
{
    ...
    /**
     * @var string 告警规则唯一id，通过接口GetAlarmRule/ListAlarmRule获取
     */
    public $AlarmRuleId;

    /**
     * @var string 告警规则新的规则名称
     */
...
项目维度监控： 项目整体任务波动告警， 7：项目波动监控告警
    ...
    /**
     * @var array 告警规则监控类型 failure：失败告警 ；overtime：超时告警 success：成功告警; backTrackingOrRerunSuccess: 补录重跑成功告警 backTrackingOrRerunFailure：补录重跑失败告警； 项目波动告警 projectFailureInstanceUpwardFluctuationAlarm： 当天失败实例数向上波动率超过阀值告警； projectFailureInstanceUpwardVolatilityAlarm：当天失败实例数向上波动量超过阀值告警 projectSuccessInstanceDownwardFluctuationAlarm：当天成功实例数向下波动率超过阀值告警； projectSuccessInstanceDownwardVolatilityAlarm： 当天成功实例数向下波动量超过阀值告警 projectFailureInstanceCountAlarm: 当天失败实例数超过阀值告警 projectFailureInstanceProportionAlarm： 当天失败实例数占比超过阀值告警 离线集成任务对账告警： reconciliationFailure： 离线对账任务失败告警 reconciliationOvertime： 离线对账任务运行超时告警 reconciliationMismatch： 数据对账任务不一致条数超过阀值告警
     */
    public $AlarmTypes;
     ...
     * @param string $AlarmRuleId 告警规则唯一id，通过接口GetAlarmRule/ListAlarmRule获取
     * @param string $AlarmRuleName 告警规则新的规则名称
...
项目维度监控： 项目整体任务波动告警， 7：项目波动监控告警
     ...
     * @param array $AlarmTypes 告警规则监控类型 failure：失败告警 ；overtime：超时告警 success：成功告警; backTrackingOrRerunSuccess: 补录重跑成功告警 backTrackingOrRerunFailure：补录重跑失败告警； 项目波动告警 projectFailureInstanceUpwardFluctuationAlarm： 当天失败实例数向上波动率超过阀值告警； projectFailureInstanceUpwardVolatilityAlarm：当天失败实例数向上波动量超过阀值告警 projectSuccessInstanceDownwardFluctuationAlarm：当天成功实例数向下波动率超过阀值告警； projectSuccessInstanceDownwardVolatilityAlarm： 当天成功实例数向下波动量超过阀值告警 projectFailureInstanceCountAlarm: 当天失败实例数超过阀值告警 projectFailureInstanceProportionAlarm： 当天失败实例数占比超过阀值告警 离线集成任务对账告警： reconciliationFailure： 离线对账任务失败告警 reconciliationOvertime： 离线对账任务运行超时告警 reconciliationMismatch： 数据对账任务不一致条数超过阀值告警
     * @param AlarmRuleDetail $AlarmRuleDetail 告警规则配置信息 成功告警无需配置；失败告警可以配置首次失败告警或者所有重试失败告警；超时配置需要配置超时类型及超时阀值；项目波动告警需要配置波动率及防抖周期配置
     * @param integer $Status 告警规则的启用状态0--禁用1--启用
     * @param integer $AlarmLevel 告警级别 1.普通、2.重要、3.紧急
     * @param array $AlarmGroups 告警接收人配置信息
     * @param string $Description 告警描述信息
     */
