 ...
 * @method string getType() 获取查询类型（string）：当前后端实现固定按 from 聚合统计，未使用该字段（保留字段，传值不会影响结果）。
 * @method void setType(string $Type) 设置查询类型（string）：当前后端实现固定按 from 聚合统计，未使用该字段（保留字段，传值不会影响结果）。
...
class DescribeDataPvUrlInfoRequest extends AbstractModel
{
    ...
    /**
     * @var string 查询类型（string）：当前后端实现固定按 from 聚合统计，未使用该字段（保留字段，传值不会影响结果）。
     */
    public $Type;
     ...
     * @param string $Type 查询类型（string）：当前后端实现固定按 from 聚合统计，未使用该字段（保留字段，传值不会影响结果）。
