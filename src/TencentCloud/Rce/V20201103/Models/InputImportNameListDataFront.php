<?php
/*
 * Copyright (c) 2017-2025 Tencent. All Rights Reserved.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *    http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
namespace TencentCloud\Rce\V20201103\Models;
use TencentCloud\Common\AbstractModel;

/**
 * 添加名单数据入参
 *
 * @method integer getNameListId() 获取名单ID
 * @method void setNameListId(integer $NameListId) 设置名单ID
 * @method integer getDataSource() 获取数据来源，固定传2（手工录入）
 * @method void setDataSource(integer $DataSource) 设置数据来源，固定传2（手工录入）
 * @method array getDataContentInfo() 获取黑白名单数据内容
 * @method void setDataContentInfo(array $DataContentInfo) 设置黑白名单数据内容
 */
class InputImportNameListDataFront extends AbstractModel
{
    /**
     * @var integer 名单ID
     */
    public $NameListId;

    /**
     * @var integer 数据来源，固定传2（手工录入）
     */
    public $DataSource;

    /**
     * @var array 黑白名单数据内容
     */
    public $DataContentInfo;

    /**
     * @param integer $NameListId 名单ID
     * @param integer $DataSource 数据来源，固定传2（手工录入）
     * @param array $DataContentInfo 黑白名单数据内容
     */
    function __construct()
    {

    }

    /**
     * For internal only. DO NOT USE IT.
     */
    public function deserialize($param)
    {
        if ($param === null) {
            return;
        }
        if (array_key_exists("NameListId",$param) and $param["NameListId"] !== null) {
            $this->NameListId = $param["NameListId"];
        }

        if (array_key_exists("DataSource",$param) and $param["DataSource"] !== null) {
            $this->DataSource = $param["DataSource"];
        }

        if (array_key_exists("DataContentInfo",$param) and $param["DataContentInfo"] !== null) {
            $this->DataContentInfo = [];
            foreach ($param["DataContentInfo"] as $key => $value){
                $obj = new DataContentInfo();
                $obj->deserialize($value);
                array_push($this->DataContentInfo, $obj);
            }
        }
    }
}
