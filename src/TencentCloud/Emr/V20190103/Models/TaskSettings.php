<?php
/*
 * Copyright (c) 2017-2025 Tencent. All Rights Reserved.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *    http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
namespace TencentCloud\Emr\V20190103\Models;
use TencentCloud\Common\AbstractModel;

/**
 * 巡检任务参数
 *
 * @method string getName() 获取参数名称
 * @method void setName(string $Name) 设置参数名称
 * @method string getValue() 获取参数值
 * @method void setValue(string $Value) 设置参数值
 * @method string getKey() 获取参数唯一标记
 * @method void setKey(string $Key) 设置参数唯一标记
 * @method string getEditable() 获取是否可编辑，”true" "false"
 * @method void setEditable(string $Editable) 设置是否可编辑，”true" "false"
 */
class TaskSettings extends AbstractModel
{
    /**
     * @var string 参数名称
     */
    public $Name;

    /**
     * @var string 参数值
     */
    public $Value;

    /**
     * @var string 参数唯一标记
     */
    public $Key;

    /**
     * @var string 是否可编辑，”true" "false"
     */
    public $Editable;

    /**
     * @param string $Name 参数名称
     * @param string $Value 参数值
     * @param string $Key 参数唯一标记
     * @param string $Editable 是否可编辑，”true" "false"
     */
    function __construct()
    {

    }

    /**
     * For internal only. DO NOT USE IT.
     */
    public function deserialize($param)
    {
        if ($param === null) {
            return;
        }
        if (array_key_exists("Name",$param) and $param["Name"] !== null) {
            $this->Name = $param["Name"];
        }

        if (array_key_exists("Value",$param) and $param["Value"] !== null) {
            $this->Value = $param["Value"];
        }

        if (array_key_exists("Key",$param) and $param["Key"] !== null) {
            $this->Key = $param["Key"];
        }

        if (array_key_exists("Editable",$param) and $param["Editable"] !== null) {
            $this->Editable = $param["Editable"];
        }
    }
}
