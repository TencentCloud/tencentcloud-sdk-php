 ...
 * @method array getJoinedGroups() 获取用户加入的用户组列表
 * @method void setJoinedGroups(array $JoinedGroups) 设置用户加入的用户组列表
...
class ListJoinedGroupsForUserResponse extends AbstractModel
{
     ...
     * @param array $JoinedGroups 用户加入的用户组列表
    ...
    public function deserialize($param)
    {
        ...
        if (array_key_exists("JoinedGroups",$param) and $param["JoinedGroups"] !== null) {
            $this->JoinedGroups = [];
            foreach ($param["JoinedGroups"] as $key => $value){
                $obj = new JoinedGroups();
                $obj->deserialize($value);
                array_push($this->JoinedGroups, $obj);
            }
        }
