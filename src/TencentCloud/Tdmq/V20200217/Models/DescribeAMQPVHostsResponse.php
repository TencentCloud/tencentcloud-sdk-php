 ...
 *
 * @method array getVHosts() 获取Vhost 列表
 * @method void setVHosts(array $VHosts) 设置Vhost 列表
...
class DescribeAMQPVHostsResponse extends AbstractModel
{
    ...
    public function deserialize($param)
    {
        ...
        if (array_key_exists("VHosts",$param) and $param["VHosts"] !== null) {
            $this->VHosts = [];
            foreach ($param["VHosts"] as $key => $value){
                $obj = new AMQPVHost();
                $obj->deserialize($value);
                array_push($this->VHosts, $obj);
            }
        }
