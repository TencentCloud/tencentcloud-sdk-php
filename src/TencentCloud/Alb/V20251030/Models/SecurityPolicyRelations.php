<?php
/*
 * Copyright (c) 2017-2025 Tencent. All Rights Reserved.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *    http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
namespace TencentCloud\Alb\V20251030\Models;
use TencentCloud\Common\AbstractModel;

/**
 * 安全策略与监听的关联关系列表。
 *
 * @method array getRelatedListeners() 获取安全策略与监听的关联关系列表。
 * @method void setRelatedListeners(array $RelatedListeners) 设置安全策略与监听的关联关系列表。
 * @method string getSecurityPolicyId() 获取安全策略 ID，格式为 tls- 后接 8 位字母数字。
 * @method void setSecurityPolicyId(string $SecurityPolicyId) 设置安全策略 ID，格式为 tls- 后接 8 位字母数字。
 */
class SecurityPolicyRelations extends AbstractModel
{
    /**
     * @var array 安全策略与监听的关联关系列表。
     */
    public $RelatedListeners;

    /**
     * @var string 安全策略 ID，格式为 tls- 后接 8 位字母数字。
     */
    public $SecurityPolicyId;

    /**
     * @param array $RelatedListeners 安全策略与监听的关联关系列表。
     * @param string $SecurityPolicyId 安全策略 ID，格式为 tls- 后接 8 位字母数字。
     */
    function __construct()
    {

    }

    /**
     * For internal only. DO NOT USE IT.
     */
    public function deserialize($param)
    {
        if ($param === null) {
            return;
        }
        if (array_key_exists("RelatedListeners",$param) and $param["RelatedListeners"] !== null) {
            $this->RelatedListeners = [];
            foreach ($param["RelatedListeners"] as $key => $value){
                $obj = new RelatedListener();
                $obj->deserialize($value);
                array_push($this->RelatedListeners, $obj);
            }
        }

        if (array_key_exists("SecurityPolicyId",$param) and $param["SecurityPolicyId"] !== null) {
            $this->SecurityPolicyId = $param["SecurityPolicyId"];
        }
    }
}
