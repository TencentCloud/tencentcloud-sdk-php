 ...
 * @method string getHostHeader() 获取自定义回源 HOST 头，该参数仅当 OriginType = IP_DOMAIN 时生效。当 OriginType 是其它类型源站时，不需要传入该参数，否则会报错。
 ...
 * @method void setHostHeader(string $HostHeader) 设置自定义回源 HOST 头，该参数仅当 OriginType = IP_DOMAIN 时生效。当 OriginType 是其它类型源站时，不需要传入该参数，否则会报错。
当 OriginType = COS 或 AWS_S3 时，回源 HOST 头将与源站域名保持一致。
当 OriginType = ORIGIN_GROUP 时，回源 HOST 头遵循源站组内配置，如果没有配置则默认为加速域名。
当 OriginType = VOD 或 SPACE 时，无需配置该头部，按对应的回源域名生效。
 ...
 * @method string getVodOriginScope() 获取云点播回源范围，该参数当 OriginType = VOD 时生效。取值有：<li>all：当前源站对应的云点播应用内所有文件；</li><li>bucket：当前源站对应的云点播应用下指定某一个存储桶内的文件，通过参数 VodBucketId 来指定存储桶。</li>不填写时，默认值为 all。
 * @method void setVodOriginScope(string $VodOriginScope) 设置云点播回源范围，该参数当 OriginType = VOD 时生效。取值有：<li>all：当前源站对应的云点播应用内所有文件；</li><li>bucket：当前源站对应的云点播应用下指定某一个存储桶内的文件，通过参数 VodBucketId 来指定存储桶。</li>不填写时，默认值为 all。
...
<li>off：不使用私有鉴权。</li>不填写时，默认值为off。
    ...
    /**
     * @var string 自定义回源 HOST 头，该参数仅当 OriginType = IP_DOMAIN 时生效。当 OriginType 是其它类型源站时，不需要传入该参数，否则会报错。
当 OriginType = COS 或 AWS_S3 时，回源 HOST 头将与源站域名保持一致。
当 OriginType = ORIGIN_GROUP 时，回源 HOST 头遵循源站组内配置，如果没有配置则默认为加速域名。
当 OriginType = VOD 或 SPACE 时，无需配置该头部，按对应的回源域名生效。
     */
...
<li>Bucket：指定的某一个存储桶。</li>
    ...
    /**
     * @var string 云点播回源范围，该参数当 OriginType = VOD 时生效。取值有：<li>all：当前源站对应的云点播应用内所有文件；</li><li>bucket：当前源站对应的云点播应用下指定某一个存储桶内的文件，通过参数 VodBucketId 来指定存储桶。</li>不填写时，默认值为 all。
     */
...
<li>off：不使用私有鉴权。</li>不填写时，默认值为off。
     ...
     * @param string $HostHeader 自定义回源 HOST 头，该参数仅当 OriginType = IP_DOMAIN 时生效。当 OriginType 是其它类型源站时，不需要传入该参数，否则会报错。
当 OriginType = COS 或 AWS_S3 时，回源 HOST 头将与源站域名保持一致。
当 OriginType = ORIGIN_GROUP 时，回源 HOST 头遵循源站组内配置，如果没有配置则默认为加速域名。
当 OriginType = VOD 或 SPACE 时，无需配置该头部，按对应的回源域名生效。
...
<li>Bucket：指定的某一个存储桶。</li>
     ...
     * @param string $VodOriginScope 云点播回源范围，该参数当 OriginType = VOD 时生效。取值有：<li>all：当前源站对应的云点播应用内所有文件；</li><li>bucket：当前源站对应的云点播应用下指定某一个存储桶内的文件，通过参数 VodBucketId 来指定存储桶。</li>不填写时，默认值为 all。
