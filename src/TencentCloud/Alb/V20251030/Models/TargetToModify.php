...
>当目标组的 **targetType** 取值为 **Instance** 时，该参数必传。
 * @method integer getWeight() 获取后端服务的权重，取值范围：**0 - 100**。如果设置权重为**0**，则不会将请求转发给该后端服务。
 * @method void setWeight(integer $Weight) 设置后端服务的权重，取值范围：**0 - 100**。如果设置权重为**0**，则不会将请求转发给该后端服务。
 */
...
>当目标组的 **targetType** 取值为 **Instance** 时，该参数必传。
    ...
    /**
     * @var integer 后端服务的权重，取值范围：**0 - 100**。如果设置权重为**0**，则不会将请求转发给该后端服务。
     */
    public $Weight;
...
>当目标组的 **targetType** 取值为 **Instance** 时，该参数必传。
     * @param integer $Weight 后端服务的权重，取值范围：**0 - 100**。如果设置权重为**0**，则不会将请求转发给该后端服务。
     */
