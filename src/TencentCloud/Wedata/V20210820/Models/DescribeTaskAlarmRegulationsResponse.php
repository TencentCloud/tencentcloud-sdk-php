...
注意：此字段可能返回 null，表示取不到有效值。
    ...
    public function deserialize($param)
    {
        ...
        if (array_key_exists("TaskAlarmInfos",$param) and $param["TaskAlarmInfos"] !== null) {
            $this->TaskAlarmInfos = [];
            foreach ($param["TaskAlarmInfos"] as $key => $value){
                $obj = new TaskAlarmInfo();
                $obj->deserialize($value);
                array_push($this->TaskAlarmInfos, $obj);
            }
        }
