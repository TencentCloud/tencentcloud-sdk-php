 ...
 * @method Models\CreateAIModelResponse CreateAIModel(Models\CreateAIModelRequest $req) 创建 AI 模型配置分组。云开发标准版及以上套餐才可使用
 ...
 * @method Models\CreateEnvResourceResponse CreateEnvResource(Models\CreateEnvResourceRequest $req) **创建环境日志资源**
 ...
 * @method Models\DescribeTableResponse DescribeTable(Models\DescribeTableRequest $req) 本接口（DescribeTable）用于查询文档型数据库表的相关信息，包括索引等信息。
 ...
 * @method Models\ModifyResourcePermissionResponse ModifyResourcePermission(Models\ModifyResourcePermissionRequest $req) 修改资源基础权限。
...
安全规则，用于控制C端用户的访问权限。详见 [安全规则介绍 ](https://cloud.tencent.com/document/product/876/123478)。
 * @method Models\ModifyStorageSourceResponse ModifyStorageSource(Models\ModifyStorageSourceRequest $req) 修改指定云开发环境已绑定的外部云存储源配置。
修改之后，大约3~5分钟生效。
 ...
 * @method Models\RollbackPGUserMigrationsResponse RollbackPGUserMigrations(Models\RollbackPGUserMigrationsRequest $req) 本接口（RollbackPGUserMigrations）用于按最近 N 条已应用 migration 倒序执行 rollback。
 ...
 * @method Models\UnbindStorageSourceResponse UnbindStorageSource(Models\UnbindStorageSourceRequest $req) 从指定云开发环境中解绑已绑定的外部云存储源。解绑后，该环境将不再关联外部 存储源，云存储功能恢复为未绑定状态。
解绑操作仅移除 CloudBase 侧的绑定关系，不会删除桶本身及桶内数据，桶仍由用户自行管理。

注意⚠️
解绑之后，会导致云存储不可用，请谨慎操作。
 * @method Models\UpdateAIModelResponse UpdateAIModel(Models\UpdateAIModelRequest $req) 更新 AI 模型配置分组。支持修改分组的模型列表、服务地址、访问密钥、备注及启用状态。
