...
注意：此字段可能返回 null，表示取不到有效值。
 * @method array getRWInstInfo() 获取代理实例信息
注意：此字段可能返回 null，表示取不到有效值。
 * @method void setRWInstInfo(array $RWInstInfo) 设置代理实例信息
注意：此字段可能返回 null，表示取不到有效值。
 */
...
注意：此字段可能返回 null，表示取不到有效值。
    ...
    public function deserialize($param)
    {
        ...
        if (array_key_exists("RWInstInfo",$param) and $param["RWInstInfo"] !== null) {
            $this->RWInstInfo = [];
            foreach ($param["RWInstInfo"] as $key => $value){
                $obj = new RWInstanceInfo();
                $obj->deserialize($value);
                array_push($this->RWInstInfo, $obj);
            }
        }
    }
}
