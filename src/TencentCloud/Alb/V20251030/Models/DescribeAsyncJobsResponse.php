 ...
 *
 * @method array getJobs() 获取任务列表。
 * @method void setJobs(array $Jobs) 设置任务列表。
 * @method integer getMaxResults() 获取分批次查询时每次显示的条目数。
 * @method void setMaxResults(integer $MaxResults) 设置分批次查询时每次显示的条目数。
 * @method string getNextToken() 获取是否拥有下一次查询的令牌（Token）。取值：  如果 NextToken 为空表示没有下一次查询。 如果 NextToken 有返回值，该取值表示下一次查询开始的令牌。
 * @method void setNextToken(string $NextToken) 设置是否拥有下一次查询的令牌（Token）。取值：  如果 NextToken 为空表示没有下一次查询。 如果 NextToken 有返回值，该取值表示下一次查询开始的令牌。
...
class DescribeAsyncJobsResponse extends AbstractModel
{
    ...
    /**
     * @var string 是否拥有下一次查询的令牌（Token）。取值：  如果 NextToken 为空表示没有下一次查询。 如果 NextToken 有返回值，该取值表示下一次查询开始的令牌。
     */
    public $NextToken;
     ...
     * @param string $NextToken 是否拥有下一次查询的令牌（Token）。取值：  如果 NextToken 为空表示没有下一次查询。 如果 NextToken 有返回值，该取值表示下一次查询开始的令牌。
