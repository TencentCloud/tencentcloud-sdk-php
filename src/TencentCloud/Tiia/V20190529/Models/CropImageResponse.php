 ...
 * @method integer getCropResult() 获取0：抠图正常, 1：原图过长, 2：原图过宽, 3：抠图区域过长, 4：抠图区域过宽, 5：纯色图 6：宽高比异常
 ...
 * @method void setCropResult(integer $CropResult) 设置0：抠图正常, 1：原图过长, 2：原图过宽, 3：抠图区域过长, 4：抠图区域过宽, 5：纯色图 6：宽高比异常
原图过长是指原图的高度是宽度的1.8倍以上；
原图过宽是指原图的宽度是高度的1.8倍以上；
抠图区域过长是指抠图的高度是主体备选框高度的1.6倍以上；
抠图过宽是指当没有检测到人脸时，抠图区域宽度是检测出的原图主体区域宽度的1.6倍以上；
...
class CropImageResponse extends AbstractModel
{
    ...
    /**
     * @var integer 0：抠图正常, 1：原图过长, 2：原图过宽, 3：抠图区域过长, 4：抠图区域过宽, 5：纯色图 6：宽高比异常
原图过长是指原图的高度是宽度的1.8倍以上；
原图过宽是指原图的宽度是高度的1.8倍以上；
抠图区域过长是指抠图的高度是主体备选框高度的1.6倍以上；
抠图过宽是指当没有检测到人脸时，抠图区域宽度是检测出的原图主体区域宽度的1.6倍以上；
...
以上是辅助决策的参考建议，可以根据业务需求选择采纳或忽视。
     ...
     * @param integer $CropResult 0：抠图正常, 1：原图过长, 2：原图过宽, 3：抠图区域过长, 4：抠图区域过宽, 5：纯色图 6：宽高比异常
原图过长是指原图的高度是宽度的1.8倍以上；
原图过宽是指原图的宽度是高度的1.8倍以上；
抠图区域过长是指抠图的高度是主体备选框高度的1.6倍以上；
抠图过宽是指当没有检测到人脸时，抠图区域宽度是检测出的原图主体区域宽度的1.6倍以上；
