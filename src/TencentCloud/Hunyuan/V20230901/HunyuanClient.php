 ...
 * @method Models\ChatTranslationsResponse ChatTranslations(Models\ChatTranslationsRequest $req) 腾讯混元大模型是由腾讯研发的大语言模型，具备强大的中文创作能力，复杂语境下的逻辑推理能力，以及可靠的任务执行能力。本接口支持流式或非流式调用，当使用流式调用时为 SSE 协议。
 ...
 3. 请使用 SDK 调用本接口，每种开发语言的 SDK Git 仓库 examples/hunyuan/v20230901/ 目录下有提供示例供参考。SDK 链接在文档下方 “**开发者资源 - SDK**” 部分提供。
 4. 我们推荐您使用 API Explorer，方便快速地在线调试接口和下载各语言的示例代码，[点击打开](https://console.cloud.tencent.com/api/explorer?Product=hunyuan&Version=2023-09-01&Action=ChatCompletions)。
 * @method Models\CreateGlossaryResponse CreateGlossary(Models\CreateGlossaryRequest $req) 创建自定义术语库，在翻译时自动将源语言术语替换为指定的目标语言译文，确保专业术语翻译的一致性和准确性。
 * @method Models\CreateGlossaryEntryResponse CreateGlossaryEntry(Models\CreateGlossaryEntryRequest $req) 添加术语条目。
 ...
 * @method Models\DeleteGlossaryEntryResponse DeleteGlossaryEntry(Models\DeleteGlossaryEntryRequest $req) 删除术语条目。
...
腾讯混元大模型是由腾讯研发的大语言模型，具备强大的中文创作能力，复杂语境下的逻辑推理能力，以及可靠的任务执行能力。本接口支持流式或非流式调用，当使用流式调用时为 SSE 协议。
 ...
 3. 请使用 SDK 调用本接口，每种开发语言的 SDK Git 仓库 examples/hunyuan/v20230901/ 目录下有提供示例供参考。SDK 链接在文档下方 “**开发者资源 - SDK**” 部分提供。
 4. 我们推荐您使用 API Explorer，方便快速地在线调试接口和下载各语言的示例代码，[点击打开](https://console.cloud.tencent.com/api/explorer?Product=hunyuan&Version=2023-09-01&Action=ChatCompletions)。
 * @method Models\ListGlossaryResponse ListGlossary(Models\ListGlossaryRequest $req) 查询术语库。
 * @method Models\ListGlossaryEntryResponse ListGlossaryEntry(Models\ListGlossaryEntryRequest $req) 查询术语条目。
 ...
 * @method Models\UpdateGlossaryEntryResponse UpdateGlossaryEntry(Models\UpdateGlossaryEntryRequest $req) 更新术语条目。
 */
