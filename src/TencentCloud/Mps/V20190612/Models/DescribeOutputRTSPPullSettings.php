 ...
 * 查询输出的RTSP拉流配置信息。
 *
 * @method array getServerUrls() 获取RTSP拉流地址列表。
注意：此字段可能返回 null，表示取不到有效值。
 * @method void setServerUrls(array $ServerUrls) 设置RTSP拉流地址列表。
注意：此字段可能返回 null，表示取不到有效值。
 */
class DescribeOutputRTSPPullSettings extends AbstractModel
{
    /**
     * @var array RTSP拉流地址列表。
注意：此字段可能返回 null，表示取不到有效值。
     */
    public $ServerUrls;

    /**
     * @param array $ServerUrls RTSP拉流地址列表。
注意：此字段可能返回 null，表示取不到有效值。
     */
    ...
    public function deserialize($param)
    {
        ...
        if (array_key_exists("ServerUrls",$param) and $param["ServerUrls"] !== null) {
            ...
            foreach ($param["ServerUrls"] as $key => $value){
                $obj = new DescribeOutputRTSPPullServerUrl();
