...
- 签署完毕的合同需要双方走解除流程将合同作废，可以参考<a href="https://qian.tencent.com/developers/partnerApis/startFlows/ChannelCreateReleaseFlow" target="_blank">发起解除合同流程接口</a>
- 撤销后可以看合同PDF内容的人员： 发起方的超管， 发起方自己，发起方撤销合同的操作人员，已经签署合同、已经填写合同、邀请填写已经补充信息的参与人员， 其他参与人员看不到合同的内容。
 * @method Models\ChannelCreateBatchQuickSignUrlResponse ChannelCreateBatchQuickSignUrl(Models\ChannelCreateBatchQuickSignUrlRequest $req) 该接口用于发起合同后，生成个人/企业用户的批量待办链接。
**注意：**
 ...
 * @method Models\ChannelCreateFlowByFilesResponse ChannelCreateFlowByFiles(Models\ChannelCreateFlowByFilesRequest $req) 本接口（ChannelCreateFlowByFiles）用于通过**已上传的 PDF 资源编号**创建待签署的合同流程。
 ...
 * @method Models\ChannelCreateFlowSignUrlResponse ChannelCreateFlowSignUrl(Models\ChannelCreateFlowSignUrlRequest $req) 该接口用于发起合同后，生成用户的签署链接 <br/>
 ...
 * @method Models\ChannelCreateOrganizationBatchSignUrlResponse ChannelCreateOrganizationBatchSignUrl(Models\ChannelCreateOrganizationBatchSignUrlRequest $req) 通过此接口，可以创建企业批量签署链接，员工只需点击链接即可跳转至控制台进行批量签署。
 ...
 * @method Models\ChannelCreatePrepareFlowGroupResponse ChannelCreatePrepareFlowGroup(Models\ChannelCreatePrepareFlowGroupRequest $req) 接口（ChannelCreatePrepareFlowGroup）用于创建嵌入式合同组签署流程。
 ...
 * @method Models\ChannelDescribeOrganizationSealsResponse ChannelDescribeOrganizationSeals(Models\ChannelDescribeOrganizationSealsRequest $req) 此接口查询子企业电子印章。<br />
 ...
 * @method Models\CreateBatchAdminChangeInvitationsUrlResponse CreateBatchAdminChangeInvitationsUrl(Models\CreateBatchAdminChangeInvitationsUrlRequest $req) 此接口用于获取企业批量变更超管链接，包含多条超管变更任务。 
一次性最多获取 500 条任务。
 ...
 * @method Models\CreateChannelOrganizationInfoChangeUrlResponse CreateChannelOrganizationInfoChangeUrl(Models\CreateChannelOrganizationInfoChangeUrlRequest $req) 此接口（CreateChannelOrganizationInfoChangeUrl）用于创建子客企业信息变更链接。
 ...
 * @method Models\CreateChannelSubOrganizationActiveResponse CreateChannelSubOrganizationActive(Models\CreateChannelSubOrganizationActiveRequest $req) 激活或续期子客企业， 在激活状态下，企业可以正常发起合同；在非激活状态下，企业仅能查看和签署合同。
 ...
 * @method Models\CreateCloseOrganizationUrlResponse CreateCloseOrganizationUrl(Models\CreateCloseOrganizationUrlRequest $req) 创建企业注销链接
 ...
 * @method Models\CreateFileConvertTaskResponse CreateFileConvertTask(Models\CreateFileConvertTaskRequest $req) 创建文件转换任务
 ...
 * @method Models\CreateFlowsByTemplatesResponse CreateFlowsByTemplates(Models\CreateFlowsByTemplatesRequest $req) 本接口（CreateFlowsByTemplates）用于**使用模板批量创建签署流程**，当前单次可批量发起合同（签署流程）数量为 1~20 个。
 ...
 * @method Models\CreateSignUrlsResponse CreateSignUrls(Models\CreateSignUrlsRequest $req) 本接口用于获取跳转至腾讯电子签小程序的签署链接，签署人点击链接后即可<font color="red">跳转到腾讯电子签小程序</font>完成合同的查看、填写与签署等操作。
...
注：
`异步任务的处理完成时间视当前已提交的任务量、任务的复杂程度等因素决定，正常情况下 3~5 秒即可完成，但也可能需要更长的时间`
 * @method Models\DescribeCancelFlowsTaskResponse DescribeCancelFlowsTask(Models\DescribeCancelFlowsTaskRequest $req) 通过接口[批量撤销合同流程](https://qian.tencent.com/developers/partnerApis/operateFlows/ChannelBatchCancelFlows)或者[获取批量撤销签署流程腾讯电子签小程序链接](https://qian.tencent.com/developers/partnerApis/operateFlows/ChannelCreateBatchCancelFlowUrl)发起批量撤销任务后，可通过此接口查询批量撤销任务的结果。
...
注: <font color='red'>所在企业的超管、法人才有权限调用此接口</font>(Agent.ProxyOperator.OpenId 需要传递超管或者法人的OpenId)
 * @method Models\DescribeFileConvertTaskResponse DescribeFileConvertTask(Models\DescribeFileConvertTaskRequest $req) 此接口（DescribeFileConvertTask）用来查询转换任务的状态。如需发起转换任务，请使用<a href="https://qian.tencent.com/developers/partnerApis/files/CreateFileConvertTask" target="_blank">创建文件转换任务接口</a>进行资源文件的转换操作<br />
 ...
 * @method Models\ModifyOrganizationBusinessInfoResponse ModifyOrganizationBusinessInfo(Models\ModifyOrganizationBusinessInfoRequest $req) 本接口用于企业完成工商变更登记后，**在电子签侧同步更新企业基础信息**，并上传最新的营业执照作为变更凭证。
...
3. 授权企业和被授权企业必须都是已认证企业
4. <font color='red'>需要授权企业或被授权企业的超管或者法人打开链接</font>走开通逻辑。
 * @method Models\OperateChannelTemplateResponse OperateChannelTemplate(Models\OperateChannelTemplateRequest $req) 此接口（OperateChannelTemplate）用于针对第三方应用平台模板库中的模板对子客企业发布授权的查询和设置。
