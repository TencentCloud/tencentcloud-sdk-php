 ...
 * @method array getEntries() 获取待删除的术语条目列表。单次最多 200 个。
 * @method void setEntries(array $Entries) 设置待删除的术语条目列表。单次最多 200 个。
 */
class DeleteGlossaryEntriesRequest extends AbstractModel
{
    ...
    /**
     * @var array 待删除的术语条目列表。单次最多 200 个。
     */
    public $Entries;

    /**
     * @param string $GlossaryId 术语库 ID。可通过 DescribeGlossaries 接口获取。
     * @param array $Entries 待删除的术语条目列表。单次最多 200 个。
     */
    ...
    public function deserialize($param)
    {
        ...
        if (array_key_exists("Entries",$param) and $param["Entries"] !== null) {
            ...
            foreach ($param["Entries"] as $key => $value){
                $obj = new DeleteGlossaryEntryInput();
