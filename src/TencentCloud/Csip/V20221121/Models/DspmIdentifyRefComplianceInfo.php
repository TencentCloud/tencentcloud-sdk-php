<?php
/*
 * Copyright (c) 2017-2025 Tencent. All Rights Reserved.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *    http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
namespace TencentCloud\Csip\V20221121\Models;
use TencentCloud\Common\AbstractModel;

/**
 * dspm关联识别模板信息
 *
 * @method integer getComplianceId() 获取<p>识别模板ID</p>
 * @method void setComplianceId(integer $ComplianceId) 设置<p>识别模板ID</p>
 * @method string getComplianceName() 获取<p>识别模板名称</p>
 * @method void setComplianceName(string $ComplianceName) 设置<p>识别模板名称</p>
 * @method integer getComplianceType() 获取<p>识别模板类型</p><p>枚举值：</p><ul><li>0： 内置</li><li>1： 自定义</li></ul>
 * @method void setComplianceType(integer $ComplianceType) 设置<p>识别模板类型</p><p>枚举值：</p><ul><li>0： 内置</li><li>1： 自定义</li></ul>
 */
class DspmIdentifyRefComplianceInfo extends AbstractModel
{
    /**
     * @var integer <p>识别模板ID</p>
     */
    public $ComplianceId;

    /**
     * @var string <p>识别模板名称</p>
     */
    public $ComplianceName;

    /**
     * @var integer <p>识别模板类型</p><p>枚举值：</p><ul><li>0： 内置</li><li>1： 自定义</li></ul>
     */
    public $ComplianceType;

    /**
     * @param integer $ComplianceId <p>识别模板ID</p>
     * @param string $ComplianceName <p>识别模板名称</p>
     * @param integer $ComplianceType <p>识别模板类型</p><p>枚举值：</p><ul><li>0： 内置</li><li>1： 自定义</li></ul>
     */
    function __construct()
    {

    }

    /**
     * For internal only. DO NOT USE IT.
     */
    public function deserialize($param)
    {
        if ($param === null) {
            return;
        }
        if (array_key_exists("ComplianceId",$param) and $param["ComplianceId"] !== null) {
            $this->ComplianceId = $param["ComplianceId"];
        }

        if (array_key_exists("ComplianceName",$param) and $param["ComplianceName"] !== null) {
            $this->ComplianceName = $param["ComplianceName"];
        }

        if (array_key_exists("ComplianceType",$param) and $param["ComplianceType"] !== null) {
            $this->ComplianceType = $param["ComplianceType"];
        }
    }
}
